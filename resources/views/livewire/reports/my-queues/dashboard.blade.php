<x-pages.index :title="__('expert-statistics::pbx.expert_statistics.my_queues_dashboard_title')">

<div class="space-y-5">

    {{-- Header row: element selector + tabs --}}
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex bg-gray-100 dark:bg-gray-800 rounded-xl p-1 gap-1">
            <button wire:click="switchTab('period')" type="button" class="px-4 py-2 text-sm font-medium rounded-lg transition-all
                    {{ $tab === 'period'
                        ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm'
                        : 'text-gray-500 dark:text-gray-400' }}">
                {{ __('expert-statistics::pbx.dashboards.period') }}
            </button>
            <button wire:click="switchTab('trend')" type="button" class="px-4 py-2 text-sm font-medium rounded-lg transition-all
                    {{ $tab !== 'period'
                        ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm'
                        : 'text-gray-500 dark:text-gray-400' }}">
                {{ __('expert-statistics::pbx.dashboards.trend') }}
            </button>
        </div>
    </div>
        ...
        @else
            ...
            @else
                @php
                    ...
                    $totalCallsTrend = $trends['totalCalls'] ?? 0;
                    $carCalls = $trends['carCalls'] ?? 0;
                    $avgWaitTrend = $trends['averageWaitTime'] ?? 0;
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10 p-4">
                        <p class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('expert-statistics::pbx.dashboards.totalCallsTitle') }}</p>
                        <p class="text-3xl font-extrabold text-teal-600 dark:text-teal-400">{{ number_format($totalCallsTrend) }}</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1 mb-2">{{ $currentMonth }} {{ $currentYear }}</p>
                        @if ($totalTrend == 0)
                            <div class="text-xs text-gray-400 dark:text-gray-500">{{ __('expert-statistics::pbx.dashboards.noChange') }} — {{ __('expert-statistics::pbx.dashboards.comparedTo', ['month' => $lastMonth]) }}</div>
                        @else
                            <div class="inline-flex items-center gap-1 text-xs font-semibold rounded-full px-2 py-1
                                {{ $totalTrend > 0 ? 'bg-green-50 dark:bg-green-950/30 text-green-600 dark:text-green-400' : 'bg-red-50 dark:bg-red-950/30 text-red-500 dark:text-red-400' }}">
                                {{ $totalTrend > 0 ? '▲' : '▼' }} {{ abs($totalTrend) }}% {{ __('expert-statistics::pbx.dashboards.comparedTo', ['month' => $lastMonth]) }}
                            </div>
                        @endif
                    </div>

                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10 p-4">
                        <p class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('expert-statistics::pbx.dashboards.answerRate') }}</p>
                        <p class="text-3xl font-extrabold text-lime-600 dark:text-lime-400">{{ $carCalls }}%</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1 mb-2">{{ $currentMonth }} {{ $currentYear }}</p>
                        @if ($carTrend == 0)
                            <div class="text-xs text-gray-400 dark:text-gray-500">{{ __('expert-statistics::pbx.dashboards.noChange') }} — {{ __('expert-statistics::pbx.dashboards.comparedTo', ['month' => $lastMonth]) }}</div>
                        @else
                            <div class="inline-flex items-center gap-1 text-xs font-semibold rounded-full px-2 py-1
                                {{ $carTrend > 0 ? 'bg-green-50 dark:bg-green-950/30 text-green-600 dark:text-green-400' : 'bg-red-50 dark:bg-red-950/30 text-red-500 dark:text-red-400' }}">
                                {{ $carTrend > 0 ? '▲' : '▼' }} {{ abs($carTrend) }}% {{ __('expert-statistics::pbx.dashboards.comparedTo', ['month' => $lastMonth]) }}
                            </div>
                        @endif
                    </div>

                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10 p-4">
                        <p class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">{{ __('expert-statistics::pbx.dashboards.avgWaitTime') }}</p>
                        <p class="text-3xl font-extrabold text-blue-600 dark:text-blue-400">{{ $this->formatDuration($avgWaitTrend) }}</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1 mb-2">{{ $currentMonth }} {{ $currentYear }}</p>
                        @if ($waitTrend == 0)
                            <div class="text-xs text-gray-400 dark:text-gray-500">{{ __('expert-statistics::pbx.dashboards.noChange') }} — {{ __('expert-statistics::pbx.dashboards.comparedTo', ['month' => $lastMonth]) }}</div>
                        @else
                            ...
                            <div class="inline-flex items-center gap-1 text-xs font-semibold rounded-full px-2 py-1
                                {{ $waitTrend > 0 ? 'bg-red-50 dark:bg-red-950/30 text-red-500 dark:text-red-400' : 'bg-green-50 dark:bg-green-950/30 text-green-600 dark:text-green-400' }}">
                                {{ $waitTrend > 0 ? '▲' : '▼' }} {{ abs($waitTrend) }}% {{ __('expert-statistics::pbx.dashboards.comparedTo', ['month' => $lastMonth]) }}
                            </div>
                        @endif
                    </div>
                </div>
                ...
                @if (! empty($answeredChart))
                    ...
                    <x-expert-statistics::charts.kpi-chart
                        :title="__('expert-statistics::pbx.dashboards.twelveMonthTrends')"
                        :rows="$trendRows"
