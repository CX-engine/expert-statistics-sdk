<div class="space-y-5">
    ...
    @php
        $loadingTargets = 'applyFilters,selectPeriod,updateTimeRange,setCustomRange,applyPbxElementSelection,selectAllPbxElements,clearPbxElements,removePbxElement';
        $perAgent = $this->getPerAgentBreakdown();
        $aggregateItems = $this->getAggregateItems();
    ...
    @else
        <div wire:loading.remove wire:target="{{ $loadingTargets }}" class="space-y-6">
            ...
            @if ($showConsolidated)
                {{-- Consolidated view: aggregate donut across all selected agents --}}
                @if (! empty($aggregateItems))
                    <x-expert-statistics::charts.donut-chart
                        :title="__('expert-statistics::pbx.expert_statistics.agent_monitoring_breakdown_aggregate_title')"
                        :items="$aggregateItems"
                        :chartKey="'agg-' . crc32(json_encode($aggregateItems))"
                    />
                @endif

                {{-- Daily stacked bar: time in each status per day, summed across all selected agents --}}
                @if (! empty($dailySeries['categories']))
                    <x-expert-statistics::charts.stacked-bar-chart
                        :title="__('expert-statistics::pbx.expert_statistics.agent_monitoring_breakdown_daily_title')"
                ...
                @endif
            @else
                {{-- Detailed view: per-agent stacked bar, time in each status --}}
                <x-expert-statistics::charts.stacked-bar-chart
                    :title="__('expert-statistics::pbx.expert_statistics.agent_monitoring_breakdown_per_agent_title')"
                    :categories="$perAgentSeries['categories']"
                    :series="$perAgentSeries['series']"
                    :chartKey="'per-agent-' . crc32(json_encode($perAgentSeries))"
                />

                {{-- Daily stacked bar per agent: small multiples, one chart per selected agent --}}
                ...
                @elseif (count($dailySeriesPerAgent) > 1)
                    <div>
                        <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-3">
                            {{ __('expert-statistics::pbx.expert_statistics.agent_monitoring_breakdown_daily_title') }}
                        </h3>
                        ...
                        <thead class="bg-gray-50 dark:bg-gray-900/50">
                            <tr class="border-b border-gray-200 dark:border-gray-700">
                                ...
                                <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                    {{ __('expert-statistics::pbx.expert_statistics.agent_monitoring_breakdown_col_status') }}
                                </th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                            ...
                            @else
                                @forelse ($perAgent as $agent)
                                    @foreach ($agent['statuses'] as $index => $status)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                            @if ($index === 0)
                                                <td rowspan="{{ count($agent['statuses']) }}" class="px-4 py-3 text-gray-800 dark:text-gray-200 whitespace-nowrap font-medium align-top border-r border-gray-100 dark:border-gray-700/50">
                                                    {{ $this->getAgentLabel($agent['user_dn']) }}
                                                </td>
                                            @endif
                                            <td class="px-4 py-3">
                                                <span class="inline-flex items-center gap-2">
                                                    <span class="w-2 h-2 rounded-full shrink-0" style="background-color: {{ $this->getStatusColor($status['status']) }}"></span>
                                                    <span class="text-gray-700 dark:text-gray-300">{{ $status['status_name'] }}</span>
                                                </span>
                                            </td>
                                            <td class="px-4 py-3 text-center text-gray-600 dark:text-gray-400 whitespace-nowrap">
                                                {{ $this->formatMinutes($status['total_minutes']) }}
                                            </td>
                                        </tr>
                                    @endforeach
                                @empty
